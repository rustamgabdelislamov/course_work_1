...
def test_determining_time_day_success():
    ...
    assert determining_time_day('2025-03-06 15:50:00') == 'Добрый день'


def test_determining_time_day_error():
    """Тест при неправильном формате даты"""
    result = determining_time_day('2025-03-06')
    assert result == 'Ошибка: неверный формат даты. Используйте формат YYYY-MM-DD HH:MM:SS.'
...
def test_cards_and_transactions_success(sample_data):
    ...
    expected = [{'last_digits': '3456', 'total_spent': 50.0, 'cashback': 0.50},
...
def test_get_currency_rates_success_no_200(mock_requests):
    ...
    result = get_currency_rates('2021-12-03 12:12:12')
    expected = f'Проблема с сетью или сервером'
    assert result == expected
